...
void CGame::GetTag(char* szInfoTag, int iInfoTag)
{
    ...
    SString strInfoTag("%c[%c%c%c] MTA: San Andreas %c:%c: %d/%d players %c:%c: %zu resources", 132, 135, szProgress[ucProgress], 132, 130, 130,
                       m_pPlayerManager->Count(), m_pMainConfig->GetMaxPlayers(), 130, 130, m_resourceManager->GetLoadedResourcesCount());
...
void CGame::DoPulse()
{
    ...
    CLOCK_CALL1(m_resourceManager->ProcessQueue(););
...
void CGame::InitialDataStream(CPlayer& Player)
{
    ...
    m_resourceManager->OnPlayerJoin(Player);
...
void CGame::Packet_PlayerScreenShot(CPlayerScreenShotPacket& Packet)
{
    ...
    if (pPlayer && pPlayer->IsJoined())
    {
        if (Packet.m_ucStatus != EPlayerScreenShotResult::SUCCESS)
        {
            ...
            if (Packet.m_resource != nullptr)
            {
                CLuaArguments Arguments;
                Arguments.PushResource(Packet.m_resource);
        ...
        else if (Packet.m_ucStatus == EPlayerScreenShotResult::SUCCESS)
        {
            ...
            if (info.bInProgress)
            {
                ...
                {
                    Resource* resource = g_pGame->GetResourceManager().GetResourceFromRemoteIdentifier(info.usResourceNetId);

                    if (resource != nullptr && info.uiTotalBytes == info.buffer.GetSize())
                    {
                        CLuaArguments Arguments;
...
//
void CGame::EnableLatentSends(bool bEnabled, int iBandwidth, CLuaMain* luaContext, ushort usResourceNetId)
{
    m_bLatentSendsEnabled = bEnabled && iBandwidth;
    m_iLatentSendsBandwidth = iBandwidth;
    m_pLatentSendsLuaMain = luaContext;
...
CMtaVersion CGame::CalculateMinClientRequirement()
{
    ...
    CMtaVersion strMinClientRequirementFromResources = m_resourceManager->GetMinClientRequirement();
